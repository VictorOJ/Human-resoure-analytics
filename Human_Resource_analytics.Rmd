...
#uSING THE SMOTE METHOD TO balance claassification
```{r}
smote_train=SMOTE(Attrition ~ .,data=trainData)
q= prop.table(table(smote_train$Attrition))
cat("Proportions are"); print(q, row.names=FALSE)
...
Raw.rf.model <- randomForest(Attrition~.,smote_train, importance=TRUE,ntree=500)
