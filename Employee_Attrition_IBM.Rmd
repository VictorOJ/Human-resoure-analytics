...
```{r}
hr_data$Yearswithoutchange <- hr_data$Yearsincurrentrole - hr_data$Yearssincelastpromotion
...
rf_varimportance <- randomForest(Attrition ~ Department + Age + Education + MaritalStatus + BusinessTravel + JobRole + DistancefromHome + DailyRate + HourlyRate + MonthlyRate + MonthlyIncome +  Numcompworked + Yearsatcompany +  Yearssincelastpromotion +  trainingtimeslastyear + stockoptionlevel + salaryhike + educationfield + jobsatisfaction + jobinvolvement + relationshipsatisfaction + worklifebalance + environmentsatisfaction + overtime + AgeGroup + OverallSatisfaction + OverallSatisfactionlevel + jobclass + Incomelevel + Yearswithoutchange, smote_trainrem, importance=TRUE,ntree=500)
...
set.seed(1337)
model_rf <- train(Attrition ~ Department + Age + Education + MaritalStatus + BusinessTravel + JobRole + DistancefromHome + DailyRate + HourlyRate + MonthlyRate + MonthlyIncome + Numcompworked + Yearsatcompany + Yearssincelastpromotion +  trainingtimeslastyear + stockoptionlevel + salaryhike + educationfield + jobsatisfaction + jobinvolvement + relationshipsatisfaction + worklifebalance + environmentsatisfaction + overtime + AgeGroup + OverallSatisfaction + OverallSatisfactionlevel + jobclass + Incomelevel + Yearswithoutchange, method = "rf", data = smote_trainrem, trControl = trainControl)
...
```{r}
set.seed(1337)
model_svm <- train(Attrition ~ Department + Age + Education + MaritalStatus + BusinessTravel + JobRole + DistancefromHome + DailyRate + HourlyRate + MonthlyRate + MonthlyIncome + Numcompworked + Yearsatcompany + Yearssincelastpromotion +  trainingtimeslastyear + stockoptionlevel + salaryhike + educationfield + jobsatisfaction + jobinvolvement + relationshipsatisfaction + worklifebalance + environmentsatisfaction + overtime + AgeGroup + OverallSatisfaction + OverallSatisfactionlevel + jobclass + Incomelevel + Yearswithoutchange, method = "svmRadial", data = smote_trainrem, trControl = trainControl)
...
```{r}
set.seed(1337)
model_glm <- train(Attrition ~ Department + Age + Education + MaritalStatus + BusinessTravel + JobRole + DistancefromHome + DailyRate + HourlyRate + MonthlyRate + MonthlyIncome +  Numcompworked + Yearsatcompany  + Yearssincelastpromotion +  trainingtimeslastyear + stockoptionlevel + salaryhike + educationfield + jobsatisfaction + jobinvolvement + relationshipsatisfaction + worklifebalance + environmentsatisfaction + overtime + AgeGroup + OverallSatisfaction + OverallSatisfactionlevel + jobclass + Incomelevel + Yearswithoutchange , method = "glm", data = smote_trainrem, trControl = trainControl, family = binomial(logit))
...
library(xgboost)
set.seed(1337)
model_xgb <- train(Attrition ~ Department + Age + Education + MaritalStatus + BusinessTravel + JobRole + DistancefromHome + DailyRate + HourlyRate + MonthlyRate + MonthlyIncome +  Numcompworked + Yearsatcompany +  Yearssincelastpromotion +  trainingtimeslastyear + stockoptionlevel + salaryhike + educationfield + jobsatisfaction + jobinvolvement + relationshipsatisfaction + worklifebalance + environmentsatisfaction + overtime + AgeGroup + OverallSatisfaction + OverallSatisfactionlevel + jobclass + Incomelevel + Yearswithoutchange, method = "xgbTree", data = smote_trainrem, trControl = trainControl)
...
```{r}
set.seed(1337)
model_lda <- train(Attrition ~ Department + Age + Education + MaritalStatus + BusinessTravel + JobRole + DistancefromHome + DailyRate + HourlyRate + MonthlyRate + MonthlyIncome + Numcompworked + Yearsatcompany + Yearssincelastpromotion +  trainingtimeslastyear + stockoptionlevel + salaryhike + educationfield + jobsatisfaction + jobinvolvement + relationshipsatisfaction + worklifebalance + environmentsatisfaction + overtime + AgeGroup + OverallSatisfaction + OverallSatisfactionlevel + jobclass + Incomelevel + Yearswithoutchange, method = "lda", data = smote_trainrem, trControl = trainControl)
...
confusionMatrix(Predictions_rf, testData$Attrition)
```


```{r}
set.seed(1337)
Predictions_glm <- predict(model_glm, testData)
...
confusionMatrix(Predictions_svm, testData$Attrition)
```

```{r}
Predictions_xgb <- predict(model_xgb, testData)

confusionMatrix(Predictions_xgb, testData$Attrition)
```

```{r}
set.seed(1337)
Predictions_lda <- predict(model_lda,testData)

confusionMatrix(Predictions_lda, testData$Attrition)
```
...
roc_rf <- roc(as.numeric(testData$Attrition), as.numeric(Predictions_rf))
roc_rf$auc

roc_svm <- roc(as.numeric(testData$Attrition), as.numeric(Predictions_svm))
roc_svm$auc

roc_xgb <- roc(as.numeric(testData$Attrition), as.numeric(Predictions_xgb))
roc_xgb$auc

roc_lda <- roc(as.numeric(testData$Attrition), as.numeric(Predictions_lda))
roc_lda$auc

roc_glm <- roc(as.numeric(testData$Attrition), as.numeric(Predictions_glm))
roc_glm$auc
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "about",
              ...
              br(),
              h3("Introduction and Significance of project"),
      ...
      tabItem(tabName = "model",
              fluidRow(
                ...
                tabBox(
                  ...
                  tabPanel("Extreme Gradient Bost", tableOutput("prediction_xgb"), textOutput("mean_xgb")),
...
ggplot(hr_data, aes(JobRole,MonthlyIncome, fill= Attrition)) + geom_boxplot() + 
  ...
  #cm_rf <- table(Truth = testData$Attrition, Pred = prediction_rf)
  #table_rf <- getPerformanceTable("Random Forest", cm_rf)
  
  cm_rf <- table(Truth = testData$Attrition, Prediction = Predictions_rf)
  
  
  output$prediction_rf <- renderTable({ cm_rf })
  output$mean_rf <- renderText({ paste("Accuracy: ", round(mean(Predictions_rf == testData$Attrition), digits = 3)) })
  
  #logistic regressision
  
  cm_glm <- table(Truth = testData$Attrition, Prediction = Predictions_glm)
  
  
  output$prediction_glm <- renderTable({ cm_glm })
  output$mean_glm <- renderText({ paste("Accuracy: ", round(mean(Predictions_glm == testData$Attrition), digits = 3)) })
  
  #Extreme gradient boost
  cm_xgb <- table(Truth = testData$Attrition, Prediction = Predictions_xgb)
  
  
  output$prediction_xgb <- renderTable({ cm_xgb })
  output$mean_xgb <- renderText({ paste("Accuracy: ", round(mean(Predictions_xgb == testData$Attrition), digits = 3)) })
  
  
  #Support vector machine
  
  cm_svm <- table(Truth = testData$Attrition, Prediction = Predictions_svm)
  
  
  output$prediction_svm <- renderTable({ cm_svm })
  output$mean_svm <- renderText({ paste("Accuracy: ", round(mean(Predictions_svm == testData$Attrition), digits = 3)) })
  
  
  #Linear discriminant model 
  
  cm_lda <- table(Truth = testData$Attrition, Prediction = Predictions_lda)
  
  
  output$prediction_lda <- renderTable({ cm_lda })
  output$mean_lda <- renderText({ paste("Accuracy: ", round(mean(Predictions_lda == testData$Attrition), digits = 3)) })
  ...
  #MOdel performance.
    ...
    pred_rf <- prediction(as.numeric(predict_rf_ROC), testData$Attrition)
  ...
  auc_rf <- round(as.numeric(auc_rf@y.values), 2)
    ...
    pred_svm <- prediction(as.numeric(predict_svm_ROC), testData$Attrition)
  ...
  auc_svm <- round(as.numeric(auc_svm@y.values), 2)
    ...
    pred_glm <- prediction(as.numeric(predict_glm_ROC), testData$Attrition)
  ...
  auc_glm <- round(as.numeric(auc_glm@y.values), 2)
    ...
    pred_lda <- prediction(as.numeric(predict_lda_ROC), testData$Attrition)
  ...
  predict_xgb_ROC <- Predictions_xgb
    pred_xgb <- prediction(as.numeric(predict_xgb_ROC), testData$Attrition)
  ...
  output$accuracy <- renderTable({
    auc <- rbind(auc_glm, auc_lda, auc_rf, auc_svm, auc_xgb)
    accuracy <- rbind(table_glm, table_lda, table_rf, table_svm, table_xgb)
